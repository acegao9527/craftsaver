...
def setup_logging():
    ...
    - EMAIL         -> src.services.email (邮件服务)
    - EMAIL_POLLING -> src.services.email_polling (邮件轮询日志)
    - CREW_EMAIL    -> src.crew.email_summary (邮件摘要 Agent)
    - TODO_REMINDER -> src.services.todo_reminder (待办提醒)
    ...
    modules_map: Dict[str, str] = {
        ...
        "src.services.telegram": "TELEGRAM",
        "src.services.telegram.polling": "TELEGRAM_POLLING",  # 轮询日志独立配置
        "src.services.email": "EMAIL",
        "src.services.email_polling": "EMAIL_POLLING",
        "src.crew.email_summary": "CREW_EMAIL",
        "src.services.todo_reminder": "TODO_REMINDER",
        "src.services.craft": "CRAFT",
        "src.services.database": "DB",
        "src.services.scheduler": "SCHEDULER",
        "src.crew_news": "CREW",
        "src.api": "API",
